...
class DestinazioniTest {
    ...
    fun `una cartella senza percorso non e una destinazione`() {
        ...
        assertEquals(
            listOf("Camera"),
            destinations(elenco, cestino).map { it.name }
        )
    }
    ...
    fun `il cestino non e una destinazione`() {
        ...
        assertEquals(listOf("Camera"), destinations(elenco, cestino).map { it.name })
    }
    ...
    fun `una cartella che somiglia al cestino resta`() {
        ...
        assertEquals(listOf("Quasi"), destinations(elenco, cestino).map { it.name })
    }

    /**
     * **Caso 4: l'ordine non si tocca.**
     *
